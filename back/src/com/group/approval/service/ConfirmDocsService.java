	...
	public static void main(String[] args) {
		...
		try {
			...
			for (Document d : selectCheckList) {
				System.out.println(d.getDocument_no() + " " + d.getDocument_title() + " "
						+ d.getEmployee().getEmployee_id() + " " + d.getEmployee().getName() + " " + d.getDraft_date()
		...
		try {
			...
			System.out.println("자신의 승인 부분 : " + list.get(0).toString() + " " + list.get(1).toString() + " "
					+ list.get(2).toString());
			System.out.println();
		} catch (FindException e) {
			// TODO Auto-generated catch block
		...
		try {
			...
			for (Document d : detailList) {
				System.out.println("문서내용 =" + d.getDocument_title() + " " + d.getDocument_type().getDocument_type()
						+ " " + d.getDocument_no() + " " + d.getEmployee().getName() + " "
						+ d.getEmployee().getEmployee_id() + " " + d.getDraft_date() + " " + d.getDocument_content()
						+ "\n결재자0=" + d.getApprovals().get(0).getEmployee_id().getName()
						+ d.getApprovals().get(0).getAp_ap_date() + "\n결재자1="
						+ d.getApprovals().get(1).getEmployee_id().getName() + d.getApprovals().get(1).getAp_ap_date()
						+ "\n결재자2=" + d.getApprovals().get(2).getEmployee_id().getName()
						+ d.getApprovals().get(2).getAp_ap_date() + "\n결재자3="
						+ d.getApprovals().get(3).getEmployee_id().getName() + d.getApprovals().get(3).getAp_ap_date()
						+ "\n합의자 =" + d.getAgreement().getEmployee_id().getName() + " "
						+ d.getAgreement().getAg_ap_date() + "\n참조자=" + d.getReference().getEmployee_id().getName());
			}
			System.out.println();
		} catch (FindException e) {
			e.printStackTrace();
		...
		try {
			...
			for (Document d : searchList) {
				System.out.println(d.getState() + " " + d.getDocument_no() + " " + d.getDocument_title() + " "
						+ d.getEmployee().getEmployee_id() + " " + d.getEmployee().getName() + " " + d.getDraft_date()
						+ " " + d.getDocument_type().getDocument_type() + " "
						+ d.getApproval().getAp_type().getApStatus_type());
			}
